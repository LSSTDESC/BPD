...
def main():
    ...
    # get data
    _data = add_noise(
        _draw_gal(), BACKGROUND, rng=np.random.default_rng(SEED), n=max_n_objs
    )
    ...
    ball_keys = random.split(ball_key, max_n_objs * N_CHAINS_PER_OBJ)
    warmup_keys = random.split(warmup_key, max_n_objs * N_CHAINS_PER_OBJ)
    sample_keys = random.split(sample_key, max_n_objs * N_CHAINS_PER_OBJ)
    ...
    # vmap twice to have multiple chains on same data over multiple data
    _run_warmup = vmap(vmap(jjit(do_warmup), in_axes=(0, 0, None)), in_axes=(0, 0, 0))
    _run_inference = vmap(
        vmap(jjit(do_inference), in_axes=(0, 0, None, 0, 0)), in_axes=(0, 0, 0, 0, 0)
    )

    # results
    results = {n * N_CHAINS_PER_OBJ: {} for n in ALL_N_OBJECTS}

    for ii, n_obj in enumerate(ALL_N_OBJECTS):
        n_chains = n_obj * N_CHAINS_PER_OBJ
        print(f"n_obj: {n_obj}")

        _keys1 = warmup_keys[:n_chains].reshape(n_obj, N_CHAINS_PER_OBJ)
        _keys2 = sample_keys[:n_chains].reshape(n_obj, N_CHAINS_PER_OBJ)
        _init_positions = {
            p: q[:n_chains].reshape(n_obj, N_CHAINS_PER_OBJ)
            for p, q in all_init_positions.items()
        }
        ...
        if ii == 0:
            ...
            (_sts, _tp), _ = jax.block_until_ready(
                _run_warmup(_keys1, _init_positions, _data_ii)
            )
            t2 = time.time()
            ...
            _ = jax.block_until_ready(
                _run_inference(
                    _keys2, _sts, _data_ii, _tp["step_size"], _tp["inverse_mass_matrix"]
                )
            )
            t2 = time.time()
    ...
    filename = f"results_benchmark2_{TAG}.npy"
