...
We slightly change the gaalxy compared to benchmark2_7
...
def main():
    ...
    # get data
    data = add_noise(
        _draw_gal(), BACKGROUND, rng=np.random.default_rng(SEED), n=N_CHAINS
    )
    ...
    _run_warmup = vmap(jjit(do_warmup), in_axes=(0, 0, 0))
    _run_inference = vmap(jjit(do_inference), in_axes=(0, 0, 0, 0, 0))
    ...
    filename = f"results_benchmark-v2_72_{N_CHAINS}_{SEED}.npy"
