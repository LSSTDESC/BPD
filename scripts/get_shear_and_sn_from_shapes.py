...
def main(
    ...
    assert Path(samples_fpath).exists(), "ellipticity samples file does not exist"
    ...
    # initialize shape noise in small ball around truth
    _sigma_e_true = ds["hyper"]["shape_noise"]
    sigma_e_initial = random.uniform(
        k1, shape=(), minval=_sigma_e_true - 0.02, maxval=_sigma_e_true + 0.02
    )

    samples = run_inference_nuts(
        k2,
        data=e1e2,
        init_positions={"g": jnp.array([0.0, 0.0]), "sigma_e": sigma_e_initial},
